...
class SamVisionAttention(nn.Module):
    ...
    def add_decomposed_rel_pos_with_micro_batch(self, attn, query, rel_pos_h, rel_pos_w, q_size, k_size):
        def process_batch(batch_indices):
            return self.add_decomposed_rel_pos(
                attn[batch_indices], 
                query[batch_indices],
                rel_pos_h,
                rel_pos_w,
        ...
        # 使用原始的 batch_size 创建一个索引张量
        batch_indices = torch.arange(attn.shape[0], device=attn.device)
        
        # 使用 micro_batch_Attention 处理索引
        return self.micro_batch_Attention(batch_indices, process_batch)
