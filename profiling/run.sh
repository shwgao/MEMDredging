...
do
    ...
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python profiling/profiling.py --model "$model" --no-batch_aggregate --no-checkpointing --offload_optimizer >> output.txt 2>&1
    echo "" >> output.txt
    echo "-------------------" >> output.txt
    ...
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python profiling/profiling.py --model "$model" --no-batch_aggregate --checkpointing --no-offload_optimizer >> output.txt 2>&1
    echo "" >> output.txt
    echo "-------------------" >> output.txt
    ...
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python profiling/profiling.py --model "$model" --batch_aggregate --checkpointing --no-offload_optimizer >> output.txt 2>&1
    echo "" >> output.txt
    echo "-------------------" >> output.txt
    ...
    echo "Config 5: batch_aggregate=false, checkpointing=false, offload_optimizer=false, torch_compile=True" >> output.txt
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python profiling/profiling.py --model "$model" --no-batch_aggregate --no-checkpointing --no-offload_optimizer --torch_compile >> output.txt 2>&1
    echo "" >> output.txt
    echo "-------------------" >> output.txt
done
